...
void	*add_node_free(t_tny *tmp, void *ptr, bool type)
{
	...
	{
		void *max = H_MED + MAX_HEADER(t_med,S_HEADER_M);
...
void	*add_small(short type, size_t lenght)
{
	...
	if (!node)
	{
		...
		dprintf(2,"=>start->start %p\n", start->start);
		dprintf(2,"tmp %p ptr %p\n",tmp, tmp->ptr);
		if (type) /// Med
		{
			/**
	...
	//	void *start = get_start(tmp->ptr,0)->start;
	//	if (!start)
	//		exit(13);
	//	void *max = (start + (type ? MAX_TINY + MAX_MED : MAX_TINY));
	//	dprintf(2,"tmp_ptr %p, max %p\n",tmp_ptr,max);
	//	size_t tmp_size = max - tmp_ptr;
	//	dprintf(2,"\ttmp_size %zu\n", tmp_size);
      //  dprintf(2,"g_mem %p\n",g_mem);
        //if (tmp_size > 4096)
        //	exit(10);
		...
		else
		{
			 ...
			 */
				if (!TINY_SIZE)
				{
					dprintf(2,"ajout add mem header tiny\n");
					add_mem_header(0);
					return (add_small(type, lenght));
				}
				tmp2 = H_TINY + MAX_HEADER(t_tny, S_HEADER_T) - TINY_SIZE;
			 ...
			 */
				if (!MED_SIZE)
				{
					dprintf(2,"ajout add tiny header med\n");
					add_mem_header(1);
					return (add_small(type, lenght));
				}
				tmp2 = H_MED + MAX_HEADER(t_med, S_HEADER_M) - MED_SIZE;
